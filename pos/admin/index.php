...
if (isset($_POST['login'])) {
  ...
  $admin_password = sha1(md5($_POST['password'])); // double encrypt to increase security
  $stmt = $mysqli->prepare("SELECT admin_id FROM rpos_admin WHERE admin_email = ? AND admin_password = ?"); // sql to log in user
  $stmt->bind_param('ss', $admin_email, $admin_password); // bind fetched parameters
  $stmt->execute(); // execute bind 
  $stmt->bind_result($admin_id); // bind result
  $stmt->fetch();
  if ($admin_id) {
    // if its successful
...
<body class="website-isi bg-dark">
        ...
        <div class="header-body text-center mb-7">
          <div class="row justify-content-center">
            <div class="col-lg-5 col-md-6">
              ...
              <?php if(isset($err)) { echo "<p class='text-white'>$err</p>"; } ?>
            </div>
          </div>
        </div>
      </div>
